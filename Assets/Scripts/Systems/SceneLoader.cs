...
public static class SceneLoader
{
    ...
    public static UniTask<T> Load<T>(RegisterSceneInInspector sceneData, LoadSceneMode mode = LoadSceneMode.Single) where T : Component
    {
        var tcs = new UniTaskCompletionSource<T>();

        // シーンのロード完了後に実行する処理を登録
        SceneManager.sceneLoaded += OnSceneLoaded;
        SceneManager.LoadScene(sceneData.Name, mode);
    ...
    /// シーンをロードする
    /// </summary>
    public static void Load(RegisterSceneInInspector sceneData, LoadSceneMode mode = LoadSceneMode.Single)
    {
        SceneManager.LoadScene(sceneData.Name, mode);
    }
